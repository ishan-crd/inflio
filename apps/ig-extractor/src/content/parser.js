...
function parseExtractionData(capturedResponses) {
  ...
  for (const resp of capturedResponses) {
    ...
    try {
      ...
      if (url.includes("/insights/account_organic/") || url.includes("/insights/creator/") || url.includes("/insights/account/")) {
...
function parseMediaInsights(data, mediaId) {
  ...
  if (root?.impressions) metrics.impressions = extractMetricValue(root.impressions);
  if (root?.engagement) metrics.engagement = extractMetricValue(root.engagement);
  ...
  if (root?.profile_visits) metrics.profileVisits = extractMetricValue(root.profile_visits);
  ...
  if (root?.inline_insights_node) {
    ...
    if (node.metrics?.impression_count) metrics.impressions = node.metrics.impression_count;
    if (node.metrics?.reach_count) metrics.reach = node.metrics.reach_count;
    if (node.metrics?.profile_visits_count) metrics.profileVisits = node.metrics.profile_visits_count;
  }
...
function parseAccountInsights(data) {
  ...
  if (root?.summary) {
    ...
    insights.accountsReached7d = extractMetricValue(s.accounts_reached_7d || s.reach_7d || s.accounts_reached);
    insights.accountsReached30d = extractMetricValue(s.accounts_reached_30d || s.reach_30d);
    insights.accountsEngaged7d = extractMetricValue(s.accounts_engaged_7d || s.engagement_7d || s.accounts_engaged);
    insights.accountsEngaged30d = extractMetricValue(s.accounts_engaged_30d || s.engagement_30d);
    insights.totalFollowers = extractMetricValue(s.total_followers || s.follower_count || s.followers_count);
  }
  ...
  if (Array.isArray(root?.metrics || root?.organic_metrics)) {
    for (const m of (root.metrics || root.organic_metrics)) {
      ...
      if (name?.includes("engaged") || name?.includes("engagement")) insights.accountsEngaged7d = val;
      if (name?.includes("follower") && name?.includes("count")) insights.totalFollowers = val;
  ...
  if (root?.engagement) insights.accountsEngaged7d = extractMetricValue(root.engagement);
  if (root?.impressions) insights.totalImpressions = extractMetricValue(root.impressions);
  if (root?.accountsReached7d) insights.accountsReached7d = root.accountsReached7d;
  if (root?.accountsEngaged7d) insights.accountsEngaged7d = root.accountsEngaged7d;
  if (root?.totalFollowers) insights.totalFollowers = root.totalFollowers;

  // Demographic breakdowns
  if (root?.gender || root?.audience_gender) {
    insights.followerGender = (root.gender || root.audience_gender).map((g) => ({
      key: g.key || g.gender,
  ...
  if (root?.top_cities || root?.audience_city) {
    insights.followerTopCities = (root.top_cities || root.audience_city).map((c) => ({
  ...
  if (root?.top_countries || root?.audience_country) {
    insights.followerTopCountries = (root.top_countries || root.audience_country).map((c) => ({
...
function extractMetricValue(metric) {
  ...
  if (typeof metric === "object") {
    ...
    if (metric.results?.[0]?.total_value != null) return metric.results[0].total_value;
  }
