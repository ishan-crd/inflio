...
(() => {
	...
	async function runExtraction() {
		...
		try {
			...
			userId =
				userData?.data?.user?.id || userData?.user?.id || userData?.user?.pk;
		...
		try {
			...
			console.log(
				"[Inflio] Reels raw response:",
				JSON.stringify(reelsData).slice(0, 500),
			);

			console.log(
				"[Inflio] Reels response keys:",
				reelsData ? Object.keys(reelsData) : "null",
			);
		...
		try {
			await igFetch(`https://www.instagram.com/api/v1/users/${userId}/info/`);
		} catch (e) {}
