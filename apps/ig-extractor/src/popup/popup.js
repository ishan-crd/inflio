...
(() => {
	...
	function populateProfile(profile) {
		...
		if (profile.followers != null)
			statFollowers.textContent = formatNum(profile.followers);
		if (profile.following != null)
			statFollowing.textContent = formatNum(profile.following);
	...
	async function startExtraction() {
		...
		try {
			...
			try {
				...
				showError(
					"Could not connect to Instagram page. Try refreshing the page.",
				);
				return;
			}
		}
	...
	async function finishExtraction() {
		...
		if (responses.length === 0) {
			showError(
				"No data was captured. Make sure you're on an Instagram profile page and you're logged in.",
			);
			return;
		}
	...
	function showResults() {
		...
		const reelsWithInsights = extractedData.reels.filter(
			(r) => r.reach > 0 || r.impressions > 0,
		).length;
