...
export default function DashboardAnalytics() {
	...
	const avgRate = allCampaigns.length > 0 ? Math.round(allCampaigns.reduce((s, c) => s + c.rate, 0) / allCampaigns.length) : 0;

	const topStats = [
		{ label: "Total spend", value: formatMoney(totalBudget, currency), change: null },
		{ label: "Total views", value: "0", change: null },
		{ label: "Avg. CPM", value: avgRate ? `${currency}${avgRate}` : "—", change: null },
	...
	allCampaigns.forEach((c) => { platformCounts[c.platform] = (platformCounts[c.platform] || 0) + 1; });
		...
		<div>
			<div className="db-page-header">
				<div>
					...
					<p className="db-page-sub">Performance metrics across all your campaigns</p>
				</div>
			</div>
			...
			<div className="db-stat-grid-6">
				{topStats.map((s) => (
					<div key={s.label} className="db-stat-card">
						<div className="db-stat-label" style={{ marginBottom: 8 }}>{s.label}</div>
			...
			<div className="db-row-2" style={{ marginTop: 20 }}>
				...
				<div className="db-card">
					...
					<div className="db-card-sub">Monthly aggregate views across campaigns</div>
					<div className="db-bar-chart">
						{months.map((m, i) => (
							<div key={m} className="db-bar-col">
								<div className="db-bar" style={{ height: 4, background: "rgba(190,242,100,0.15)" }} />
								<span className="db-bar-label">{m}</span>
							</div>
						))}
					</div>
					<div style={{ textAlign: "center", padding: "20px 0 0", fontSize: 12, color: "var(--color-ink-3)" }}>
				...
				<div className="db-card">
					...
					<div className="db-card-sub">Aggregate reactions across submissions</div>
					<div className="db-engagement-grid">
						{[
							...
							<div key={e.label} className="db-engagement-item">
								<div className="db-engagement-num" style={{ color: e.color }}>{e.value}</div>
			...
			<div className="db-card" style={{ marginTop: 20 }}>
				<div className="db-card-title" style={{ marginBottom: 20 }}>Campaign performance</div>
				{allCampaigns.length === 0 ? (
					<div className="db-empty" style={{ padding: "40px 0" }}>
						<div className="db-empty-title">No campaign data</div>
						<div className="db-empty-desc">Create a campaign to start tracking performance</div>
					</div>
				) : (
										...
										<tr key={c._id}>
											<td>
												<div style={{ display: "flex", alignItems: "center", gap: 10 }}>
													<div style={{ width: 8, height: 8, borderRadius: "50%", background: COLORS[c.color] || "#bef264" }} />
													<span style={{ fontWeight: 500 }}>{c.title}</span>
												</div>
											</td>
											<td><span className={`db-status-badge ${c.status}`}>{c.status}</span></td>
											<td>{formatMoney(c.budget, c.currency)}</td>
											<td>{c.currency}0</td>
											<td>{c.creatorsJoined}/{c.totalSpots}</td>
											<td>0</td>
											<td>0</td>
											<td>{c.currency}{c.rate}</td>
											<td>
												<div style={{ display: "flex", alignItems: "center", gap: 8 }}>
													...
													<span style={{ fontSize: 11, color: "var(--color-ink-2)" }}>{fillPct}%</span>
												</div>
											</td>
										</tr>
									);
			...
			<div className="db-card" style={{ marginTop: 20 }}>
				<div className="db-card-title" style={{ marginBottom: 6 }}>Top performing creators</div>
				<div className="db-card-sub">Ranked by views generated for your campaigns</div>
				<div className="db-empty" style={{ padding: "40px 0" }}>
					...
					<div className="db-empty-desc">Creator rankings will appear once submissions start coming in</div>
				</div>
			</div>
		</div>
	);
