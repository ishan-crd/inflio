...
const COLORS: Record<string, string> = {
	lime: "#bef264", cyan: "#67e8f9", violet: "#c084fc", orange: "#fb923c", rose: "#fb7185", blue: "#60a5fa",
};
...
export default function DashboardHome() {
	...
	const totalBudget = allCampaigns.reduce((sum, c) => sum + Number(c.budget?.replace(/[^0-9]/g, "") || 0), 0);
	const totalSpots = allCampaigns.reduce((sum, c) => sum + (c.totalSpots || 0), 0);
	const filledSpots = allCampaigns.reduce((sum, c) => sum + (c.creatorsJoined || 0), 0);
	...
	const stats = [
		{
			...
			icon: (
				<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--color-accent-strong)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
		...
		},
		{
			...
			value: "0",
			icon: (
				<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#f472b6" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
				...
				<div className="db-card">
					...
					<div className="db-donut-wrap">
						<svg viewBox="0 0 200 200" className="db-donut">
							{/* Background ring */}
							<circle cx="100" cy="100" r="70" fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth="32" />
							{allCampaigns.length === 0 ? (
								<>
									{/* Empty state segments */}
									<circle cx="100" cy="100" r="70" fill="none" stroke="#4ade80" strokeWidth="32" strokeDasharray="165 275" strokeDashoffset="0" opacity="0.3" strokeLinecap="round" />
									<circle cx="100" cy="100" r="70" fill="none" stroke="#fb7185" strokeWidth="32" strokeDasharray="110 330" strokeDashoffset="-165" opacity="0.3" strokeLinecap="round" />
									<circle cx="100" cy="100" r="70" fill="none" stroke="#fbbf24" strokeWidth="32" strokeDasharray="110 330" strokeDashoffset="-275" opacity="0.3" strokeLinecap="round" />
								</>
							) : null}
						</svg>
				...
				<div className="db-card">
					...
					<div className="db-platform-list">
						{platforms.map((p) => {
							...
							const pct = allCampaigns.length > 0 ? Math.round((count / allCampaigns.length) * 100) : 0;
							return (
								<div key={p.name} className="db-platform-row">
									...
									<div className="db-platform-bar">
										<div className="db-platform-bar-fill" style={{ width: `${pct}%`, background: p.color }} />
									</div>
								</div>
							);
						})}
			...
			<div className="db-card" style={{ marginTop: 20 }}>
				<div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
					<div>
						...
						<div className="db-card-sub">Monthly view volume with growth indicators</div>
					</div>
					<div className="db-legend">
						<div className="db-legend-item"><div className="db-legend-dot" style={{ background: "#4ade80" }} /><span>This campaign</span></div>
						<div className="db-legend-item"><div className="db-legend-dot" style={{ background: "#fbbf24" }} /><span>Peer campaign</span></div>
					</div>
				</div>
				...
				{allCampaigns.length === 0 ? (
					<div className="db-empty-chart">
						<div className="db-empty-icon">
							<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--color-ink-3)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
						...
						<div className="db-empty-desc">Create a campaign to start tracking.</div>
					</div>
				) : (
					<div className="db-empty-chart">
						<div className="db-empty-title">Data will appear as creators submit content</div>
					</div>
				)}
			</div>
			...
			{activeCampaigns.length > 0 && (
				<div className="db-card" style={{ marginTop: 20 }}>
					<div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
						...
						<Link href="/dashboard/campaigns" style={{ fontSize: 12, color: "var(--color-accent-strong)", textDecoration: "none" }}>
							View all
						</Link>
					</div>
							...
							<tbody>
								{activeCampaigns.slice(0, 5).map((c) => (
									<tr key={c._id}>
										...
										<td>{c.creatorsJoined}/{c.totalSpots}</td>
										<td style={{ color: daysUntil(c.deadline) <= 3 ? "#fb7185" : "inherit" }}>
											{daysUntil(c.deadline)}d left
										</td>
										<td><span className="db-status-badge active">Active</span></td>
									</tr>
								))}
							</tbody>
						</table>
...
function PlatformIcon({ name }: { name: string }) {
	if (name === "ig") return (
		<svg width="18" height="18" viewBox="0 0 24 24" fill="none">
			<rect x="2" y="2" width="20" height="20" rx="5" stroke="#e040fb" strokeWidth="1.5" />
	...
	if (name === "yt") return (
		<svg width="18" height="18" viewBox="0 0 24 24" fill="none">
			<rect x="2" y="4" width="20" height="16" rx="4" stroke="#ff1744" strokeWidth="1.5" />
			<polygon points="10,8 16,12 10,16" fill="#ff1744" />
		</svg>
	);
	if (name === "tt") return (
		<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#00e5ff" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
