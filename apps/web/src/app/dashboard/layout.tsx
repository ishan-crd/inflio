...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
	...
	if (!brandProfile) {
		return (
			<div className="db-loading">
				<p style={{ color: "var(--color-ink-2)", marginBottom: 16 }}>Complete your brand onboarding to access the dashboard.</p>
				<Link href="/onboarding?role=brand" className="btn btn-primary" style={{ textDecoration: "none" }}>
					Complete Onboarding
				</Link>
			</div>
		);
	...
	const navItems = [
		...
		{ href: "/dashboard/verifications", label: "Verifications", icon: "verify" },
	];
		...
		<div className={`db-layout${collapsed ? " db-collapsed" : ""}`}>
			...
			<aside className="db-sidebar">
				<div className="db-sidebar-top">
					{/* Logo */}
					<Link href="/" className="db-logo" style={{ textDecoration: "none", color: "inherit" }}>
				...
				<div className="db-sidebar-bottom">
					...
					<button className="db-nav-item" onClick={() => setCollapsed(!collapsed)}>
					...
					<div className="db-brand-card">
						<div className="db-brand-avatar" style={{
							background: brandProfile.logoColors?.[0] || "linear-gradient(135deg, var(--color-accent-strong), #65a30d)",
						...
						{!collapsed && (
							<div className="db-brand-info">
								...
								<div className="db-brand-meta">{campaigns?.length ?? 0} Campaign{(campaigns?.length ?? 0) !== 1 ? "s" : ""}</div>
							</div>
						)}
					</div>
				</div>
			...
			<div className="db-main">
				...
				<header className="db-topbar">
					<div className="db-search">
						<svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="var(--color-ink-3)" strokeWidth="1.5" strokeLinecap="round">
...
function NavIcon({ name }: { name: string }) {
	...
	switch (name) {
		...
		case "marketplace":
			return <svg {...p}><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 7V5a4 4 0 00-8 0v2"/></svg>;
		case "campaigns":
		...
		case "settings":
			return <svg {...p}><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83 0l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-4 0v-.09A1.65 1.65 0 009 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83 0 2 2 0 010-2.83l.06-.06A1.65 1.65 0 004.68 15a1.65 1.65 0 00-1.51-1H3a2 2 0 010-4h.09A1.65 1.65 0 004.6 9a1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 112.83-2.83l.06.06A1.65 1.65 0 009 4.68a1.65 1.65 0 001-1.51V3a2 2 0 014 0v.09a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 012.83 2.83l-.06.06A1.65 1.65 0 0019.4 9a1.65 1.65 0 001.51 1H21a2 2 0 010 4h-.09a1.65 1.65 0 00-1.51 1z"/></svg>;
		case "verify":
			return <svg {...p}><path d="M9 12l2 2 4-4"/><circle cx="12" cy="12" r="9"/></svg>;
		case "collapse":
			return <svg {...p}><polyline points="11 17 6 12 11 7"/><polyline points="18 17 13 12 18 7"/></svg>;
		case "expand":
			return <svg {...p}><polyline points="13 17 18 12 13 7"/><polyline points="6 17 11 12 6 7"/></svg>;
		default:
			return null;
	}
}
